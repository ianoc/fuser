...
use crate::{ReplyFailed, channel, fuse_abi::consts::*, reply::{ReplySender2, Replyable}};
use crate::fuse_abi::*;
...
#[async_trait]
impl ReplySender2 for SingleSender {
...
impl<'a> Request<'a> {
    ...
    pub fn new(data: Vec<u8>) -> Option<Request<'a>> {
        // eprintln!("In request,,, bufer len: {}", data.len());
    ...
    pub async fn dispatch_init<FS: Filesystem>(&self, se: &mut Session<FS>,
        ch: ChannelSender,
    ) -> bool {
        debug!("{}", self.request);
        ...
        match self.request.operation() {
            ...
            ll::Operation::Init { arg } => {
                ...
                let res = se.filesystem.as_mut().unwrap().init(self, &mut config);
                if let Err(err) = res {
            ...
            _  => {
                ...
                self.reply::<ReplyEmpty>(&ch).error(EIO);
                false
            }
    ...
    pub async fn maybe_destroy_dispatch<FS: Filesystem>(&self, se: &mut Session<FS>,ch: ChannelSender,) -> bool {
        debug!("{}", self.request);
        match self.request.operation() {
    ...
    pub async fn dispatch<FS: Filesystem>(&self, filesystem: Arc<FS>,ch: ChannelSender,) -> () {
        debug!("{}", self.request);

        match self.request.operation() {
            // Filesystem initialization
            ll::Operation::Init { arg } => {
                warn!("Already initialized, got init after init init: {}", self.request);
                self.reply::<ReplyEmpty>(&ch).error(EIO);
            }

            ll::Operation::Interrupt { .. } => {
                // TODO: handle FUSE_INTERRUPT
                self.reply::<ReplyEmpty>(&ch).error(ENOSYS);
            }

            ll::Operation::Lookup { name } => {
                self.send(&ch, filesystem
                    .lookup(self, self.request.nodeid(), &name).await).await;
            }
            ...
            ll::Operation::GetAttr => {
                filesystem
                    .getattr(self, self.request.nodeid(), self.reply(&ch)).await;
            }
            ...
            ll::Operation::ReadLink => {
                filesystem
                    .readlink(self, self.request.nodeid(), self.reply(&ch)).await;
            }
            ...
            ll::Operation::Unlink { name } => {
                filesystem
                    .unlink(self, self.request.nodeid(), &name, self.reply(&ch)).await;
            }
            ll::Operation::RmDir { name } => {
                filesystem
                    .rmdir(self, self.request.nodeid(), &name, self.reply(&ch)).await;
            }
            ...
            ll::Operation::Open { arg } => {
                filesystem
                    .open(self, self.request.nodeid(), arg.flags, self.reply(&ch)).await;
            }
            ...
            ll::Operation::FSync { arg } => {
                ...
                filesystem
                    .fsync(self, self.request.nodeid(), arg.fh, datasync, self.reply(&ch)).await;
            }
            ll::Operation::OpenDir { arg } => {
                filesystem
                    .opendir(self, self.request.nodeid(), arg.flags, self.reply(&ch)).await;
            }
            ...
            ll::Operation::FSyncDir { arg } => {
                let datasync = !matches!(arg.fsync_flags & 1, 0);
                filesystem
                    .fsyncdir(self, self.request.nodeid(), arg.fh, datasync, self.reply(&ch)).await;
            }
            ll::Operation::StatFs => {
                filesystem
                    .statfs(self, self.request.nodeid(), self.reply(&ch)).await;
            }
            ...
            ll::Operation::GetXAttr { arg, name } => {
                filesystem
                    .getxattr(self, self.request.nodeid(), name, arg.size, self.reply(&ch)).await;
            }
            ll::Operation::ListXAttr { arg } => {
                filesystem
                    .listxattr(self, self.request.nodeid(), arg.size, self.reply(&ch)).await;
            }
            ll::Operation::RemoveXAttr { name } => {
                filesystem
                    .removexattr(self, self.request.nodeid(), name, self.reply(&ch)).await;
            }
            ll::Operation::Access { arg } => {
                filesystem
                    .access(self, self.request.nodeid(), arg.mask, self.reply(&ch)).await;
            }
            ...
            ll::Operation::BatchForget { arg: _, nodes } => {
                filesystem.batch_forget(self, nodes); // no reply
            }
    ...
    async fn send<T: Replyable>(&self, ch: &ChannelSender, t: Result<T, ReplyFailed>) -> std::io::Result<()> {
        let sender = SingleSender(ch.clone());
        match t {
            Ok(t) => {
                t.send(self.request.unique(), sender).await
            }
            Err(t) => {
                t.send(self.request.unique(), sender).await
            }
