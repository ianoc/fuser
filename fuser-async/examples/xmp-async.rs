...
impl Filesystem for XmpFS {
    async fn init(
        &self,
        _req: &Request<'_>,
        config: &mut fuser_async::KernelConfig,
    ) -> Result<(), c_int> {
...
async fn main() {
    ...
    let options = [
        fuser_async::MountOption::RW,
        fuser_async::MountOption::Async,
        fuser_async::MountOption::FSName(String::from("xmp")),
        fuser_async::MountOption::AutoUnmount,
    ];
